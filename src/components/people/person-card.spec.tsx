...
describe("PersonCard", () => {
  it("should render image when it exists", async () => {
    await render(<PersonCard person={person} />);

    const image = screen.getByRole("img", { name: person.name });
  ...
  it("should render known for", async () => {
    ...
    const text = screen.getByText(`Known for ${person.known_for_department}`);
