...
- Interface work follows [TASTE.md](TASTE.md). Read it before changing UI.
- Use shadcn/ui components for all UI primitives (buttons, inputs, dialogs, etc.). Do not reimplement these.
- Files in `src/components/ui/**` are generated by the shadcn CLI. Never edit them directly. If a component needs different behavior, stop and ask before proceeding.
- Keep custom CSS and Tailwind on shadcn components minimal. Prefer shadcn defaults. If you think custom styling is needed, stop and ask before writing it.
- If a design requires something shadcn does not obviously provide, first check the shadcn/ui registry (https://ui.shadcn.com/docs/components) before building anything custom. If nothing in the registry fits, stop and ask before building custom.
